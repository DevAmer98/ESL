...
/**
 * Recent operation-log rows for a store (label objectType). Each row carries
 * actionType (1 = refresh) and result ("1" = refresh success) plus createTime —
 * enough to compute refresh success/failure and weekly buckets.
 */
export async function listOperationLogs(config, minewStoreId, { pageSize = 500 } = {}) {
  const raw = await call(config, "logs", {
    body: {
      currentPage: 1,
      pageSize,
      condition: "",
      storeId: minewStoreId,
      objectType: "1",
      actionType: "",
    },
  });
