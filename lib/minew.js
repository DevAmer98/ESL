...
async function call(config, pathKey, { method = "POST", body, query, rawQuery, _retriedAuth = false } = {}) {
  ...
  // rawQuery is appended verbatim (e.g. eqstatus=1,2,5,8,9 — Minew wants literal
...
export async function fetchLabelStates(settings) {
  ...
  const raw = await call(config, "labels", {
    method: "GET",
    rawQuery: `page=1&size=500&storeId=${encodeURIComponent(config.storeId)}&eqstatus=1,2,5,8,9&type=1`,
  });
  return raw?.items ?? [];
}
...
export async function listTagsFor(config, minewStoreId) {
  const raw = await call(config, "labels", {
    method: "GET",
    rawQuery: `page=1&size=500&storeId=${encodeURIComponent(minewStoreId)}&eqstatus=1,2,5,8,9&type=1`,
  });
