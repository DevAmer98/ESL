...
export function configFromSettings(settings) {
  ...
  return {
    baseUrl: settings.cloudUrl.replace(/\/+$/, ""),
    username,
    password,
    storeId,
 ...
 */
async function call(config, pathKey, { method = "POST", body, query, _retriedAuth = false } = {}) {
...
export async function fetchLabelStates(settings) {
  ...
  const raw = await call(config, "labels", {
    method: "GET",
    query: { page: "1", size: "500", storeId: config.storeId, eqstatus: "1,2,5,8,9", type: "1" },
  });
  return raw?.items ?? [];
}
...
export function accountConfig() {
  ...
  return {
    baseUrl: (process.env.MINEW_URL ?? "https://esl.smartvisionss.com:9443").replace(/\/+$/, ""),
    username,
    password,
    paths: { ...DEFAULT_PATHS },
